...
class AdbController:
    ...
    def _normalize_to_logical_resolution(self, img):
        """将截图等比缩放至 1600×900 逻辑分辨率（stretch-to-fit）。
        
        存储设备原始分辨率供坐标反算，保持与旧版一致的坐标语义。
        """
        import cv2
        if img is None:
            return None
        ...
        if w == self.LOGICAL_WIDTH and h == self.LOGICAL_HEIGHT:
            return img
        interp = cv2.INTER_AREA if w > self.LOGICAL_WIDTH else cv2.INTER_LINEAR
