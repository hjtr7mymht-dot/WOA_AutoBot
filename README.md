...
- 程序会在启动阶段自动进行一次联网版本检测（GitHub Raw -> jsDelivr -> ghproxy）。
- 检测到新版本时仅提示，不会自动下载、覆盖或重启。
...
- 启动时也会自动执行一次更新检测，并在状态栏展示检测结果来源（GitHub Raw / jsDelivr / ghproxy）。

推荐发布流程：

1. 本地更新版本号与代码。
2. 重新打包生成 [dist/WOA_AutoBot.zip](dist/WOA_AutoBot.zip)。
3. 将 zip 上传到 GitHub Releases，例如 `v1.1.0`。
