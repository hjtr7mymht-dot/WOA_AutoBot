# WOA AutoBot (世界机场游戏辅助脚本) ✈️

> **声明：此脚本完全免费！若您通过付费渠道获得，请立即退款并举报。**
> **官方交流与反馈：QQ群 1067076460 联系作者3176719407**

WOA AutoBot 是一款专为《World of Airports》世界机场游戏开发的自动化辅助工具。旨在通过图像识别技术，帮助玩家处理繁琐的机场地面任务，提升挂机效率。

开发者仅为初学者，水平有限，并大量使用了Cursor、Calude、Gemini等AI工具辅助开发。

## 🤝 致谢
此为二创重置项目 原项目链接为https://github.com/nj-yzf/WOA_AutoBot
已取得原项目作者授权修改 BY18°Ray
当前官方仓库为 https://github.com/hjtr7mymht-dot/WOA_AutoBot
本项目在开发过程中，模拟器连接与控制部分的逻辑设计大量参考了 [ALAS (AzurLaneAutoScript)](https://github.com/LmeSzinc/AzurLaneAutoScript)。
感谢 ALAS 开发团队为开源社区做出的贡献。

---

## 🌟 核心特性

- **极致速度**：支持 `nemu_ipc` (MuMu 专用) 和 `uiautomator2` 高速截图技术，识别响应极快。
- **多种触控**：支持 ADB 与 `uiautomator2` 两种触控方式，兼顾兼容性与速度。
- **自适应分辨率**：自动将任意模拟器横屏分辨率映射到逻辑分辨率，兼容不同分辨率设备。
- **模块化执行**：生命周期、任务扫描、空闲恢复与各任务类型均支持独立开关，可按分支组合运行。
- **在线验证**：支持连接官方仓库进行在线校验，并在 GitHub 不可达时自动回退到国内镜像链路。
- **拟人化设计**：内置“随机点击偏移”、“随机思考时间”、“随机任务序列”和“滑动随机起始位置和用时”，大幅降低封号风险。
- **全自动化**：自动领取免费地勤、自动购买车辆、自动延时塔台、自动调整筛选。
- **多端兼容**：适配 MuMu12/6、雷电、蓝叠等主流 Windows 安卓模拟器。

这是一张GUI预览图：

<img width="684" height="891" alt="image" src="https://github.com/user-attachments/assets/aad4bd6f-8e83-4e62-b5fe-3de9029c9742" />

---

## 🛠️ 环境准备

1. **操作系统**：Windows 10/11
2. **模拟器**：强烈推荐使用 **MuMu 模拟器 12**。
3. **分辨率设置**：建议横屏分辨率（不再强制固定 `1600x900`，脚本会自动适配）。
4. **游戏语言**：必须设置为 **简体中文**。
5. **在线验证**：脚本默认优先访问 GitHub Raw，若处于中国特殊网络环境，会自动尝试 jsDelivr 与 ghproxy 方案。

---

## 🚀 快速开始

1. 从 [Releases] 页面下载打包好的 `WOA_AutoBot.zip`。
2. 运行模拟器并进入游戏机场界面。
3. 打开脚本，点击“智能扫描”，选择你的模拟器。
4. 点击“启动脚本”。
