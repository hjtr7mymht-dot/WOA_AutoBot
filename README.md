...
- 支持按类别轮换处理飞机（喜爱、机队、其他玩家、活动飞机等，多选）- 支持深色/浅色模式手动切换，主题偏好自动保存
- 支持分辨率自适应坐标缩放（设备物理坐标直连，避免二次缩放偏差）- 支持方案自检与自动回退到可用的触控/截图组合
- 支持防卡死多次触发后的自动停机保护
...
### v1.3.0 (2026-06-05)
- 🌙 **新增深色模式**：支持深色/浅色模式手动切换，主题偏好自动保存，重启后保持
- 📂 **类别栏处理全面重构**：修复类别切换逻辑，支持像素验证、分辨率自适应、防卡死误判过滤
- ⚙️ **类别轮换智能控制**：仅勾选 1 个类别时自动关闭轮换开关，取消全部类别时自动恢复"全部待处理"
- 🔧 **Mixin 继承链清理**：移除 ConfigMixin/TowerMixin 死继承，方法内联至 WoaBot 类
